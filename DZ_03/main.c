...
// очищенная строка из файла, в которой удалены (),.  и переведённая в нижний регистр букв
wchar_t temp_w_буфер[РАЗМЕР_W_БУФЕРА]=L"";  
    
// массив для вычисления хэш суммы
...
struct структ_хэш_записи {
    ...
    int повторов;
    bool есть_коллизия; // true если этудля этого хэш ключа есть коллизия (одинаковая хэш сумма для разных фраз)
};


// Очистка слова от мусора
int f_очистка_слова(wchar_t *буфер_для_вывода);

// хэш сумма для слова 
unsigned long long int f_хэш_функция(wchar_t *temp_w_буфер);
...
int main(int argc, char **argv)
{
    ...
    struct структ_хэш_записи *гл_структура_хэш_записи; // 
    ...
    гл_структура_хэш_записи = calloc(размер_хэш_таблицы, sizeof(struct структ_хэш_записи)); 
    ...
    // как глобальная переменная = unsigned long long int arr_степени_ключа[СТЕПЕНЬ_КЛЮЧА+1]; // 13 в 17 степени
    ...
    while (цикл == true) 
    {
        ...
        // Очистка слова от мусора
        f_очистка_слова(буфер_для_вывода);
        // наполняет глобальную переменную  =  temp_w_буфер  данными
        
        // хэш сумма для слова 
        хэш_сумма = f_хэш_функция(temp_w_буфер);
        ...
        {
            // первая запись в хэш таблице/структуре
            if (0==гл_структура_хэш_записи[id].повторов)
            {
                ...
                гл_структура_хэш_записи[id].есть_коллизия=false;
            }
            else
            {
                // Проверки функции на коллизии
                if (0!=wcscmp(гл_структура_хэш_записи[id].текст, temp_w_буфер)) // возникла коллизия 
                {
                    гл_структура_хэш_записи[id].есть_коллизия = true; 
                    коллизий_раз ++;
                    
                    printf("\nДля НОВОГО слова '%ls' образовалась коллизия со старой записью '%ls' (id=%i) ", temp_w_буфер, 
                        гл_структура_хэш_записи[id].текст, id);
                    
                    for (int i2=id+1; i2<размер_хэш_таблицы; i2++) // ищем первый пустой индекс для внесения записи
                    {
                        if (0 == wcscmp(гл_структура_хэш_записи[i2].текст, temp_w_буфер))
                        {                                    
                            // Коллизию уже добавляли ранее, просто увеличиваем счётчик
                            гл_структура_хэш_записи[i2].повторов ++;
                            break;
                        }
                        else if (0==гл_структура_хэш_записи[i2].повторов)
                        {
                            гл_структура_хэш_записи[i2].есть_коллизия = true; 
                                    // это запись сформирована не по хэшу, значит может быть коллизия по чистому хэшу. 
                                    // Нужно при чтении данных из хэш таблицы
                                    
                            гл_структура_хэш_записи[i2].повторов = 1;
                            wcscpy(гл_структура_хэш_записи[i2].текст, temp_w_буфер); 
                            
                            printf(" \t записано в new_id = %i\n\n ", i2); 
                            break;
                        }
                    }
                }
                else // текущее слово полностью совпадает с тем, что уже есть в нашей хэш таблице
                {
                    гл_структура_хэш_записи[id].повторов += 1;
                }
            }               
    ...
    printf("\nВ Выборке возникло 'Коллизий' раз: %i\n\n", коллизий_раз);
...
int f_очистка_слова(wchar_t *буфер_для_вывода)
{
    // printf("очистка слова от мусора\n");
    extern wchar_t temp_w_буфер[РАЗМЕР_W_БУФЕРА];
...
{
    unsigned long long int хэш_сумма=0;
    extern unsigned long long int arr_степени_ключа[СТЕПЕНЬ_КЛЮЧА+1];
    
    for (int i=0; i<wcslen(temp_w_буфер); i++)
    {
        //ПРИМЕЧАНИЕ: h(S)  =  S[0]  +  S[1] * P  +  S[2] * P^2  +  S[3] * P^3  +  ...  +  S[N] * P^N
        хэш_сумма = (хэш_сумма + temp_w_буфер[i] - L'a' + 1) * arr_степени_ключа[i];
        //printf("%llu\n", хэш_сумма);
    }
